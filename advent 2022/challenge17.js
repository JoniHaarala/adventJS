...
// []
// en cada saco se puede llevar maximo 2kg
// por lo tanto los sacos no pueden llevar ningun juguete

function carryGifts(gifts, maxWeight) {
    
    gifts = gifts.filter(x => x.length <= maxWeight)
    let bolsa = []
    let auxArr = []
    let totalWeigth = 0
    
    gifts.forEach(item => {
        totalWeigth += item.length
        if(totalWeigth <= maxWeight) 
        {
            auxArr.push(item)
            return
        } 
        bolsa.push(auxArr.join(' '))
        totalWeigth = item.length
        auxArr = [item]
    })
    bolsa.push(auxArr.join(' '))
    
  return bolsa.filter(y => y!='')
}